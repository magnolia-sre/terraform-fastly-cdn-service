resource "fastly_tls_subscription" "service" {
  count                 = var.fastly_service_tls_enable_with_aws.enable ? 1 : 0
  domains               = fastly_service_vcl.service.domain[*].name
  certificate_authority = var.fastly_service_tls_enable_with_aws.certificate_authority
  force_update          = var.fastly_service_tls_enable_with_aws.force_update
  force_destroy         = var.fastly_service_tls_enable_with_aws.force_destroy
}
...
resource "aws_route53_record" "service" {
  count   = var.fastly_service_tls_enable_with_aws.enable ? 1 : 0
  name    = var.domain
  type    = var.fastly_service_tls_enable_with_aws.route_53_record["type"]
  ...
  ttl     = var.fastly_service_tls_enable_with_aws.route_53_record["ttl"]
}
...
resource "aws_route53_record" "service_validation" {
  ...
  zone_id         = data.aws_route53_zone.magnolia_cloud_hosted_zone[0].id
  allow_overwrite = var.fastly_service_tls_enable_with_aws.route_53_validation["allow_overwrite"]
  records         = [fastly_tls_subscription.service[0].managed_dns_challenge.record_value]
  ttl             = var.fastly_service_tls_enable_with_aws.route_53_validation["ttl"]
}
